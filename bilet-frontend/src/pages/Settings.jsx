      ...
      <div className="space-y-6">
        <div className="bg-white p-8 rounded-3xl border border-orange-100 shadow-lg shadow-orange-900/5 relative overflow-hidden">
          ...
          <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
            <svg className="w-6 h-6 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>
            Görünüm
          </h2>
          <div className="flex items-center justify-between">
            <div>
              <p className="font-bold text-gray-700">Karanlık Mod</p>
              <p className="text-sm text-gray-500">Gözleri yormayan karanlık tema (Yakında)</p>
            </div>
            <button 
              onClick={() => setDarkMode(!darkMode)}
              className={`w-14 h-7 flex items-center rounded-full p-1 transition-colors duration-300 ease-in-out ${darkMode ? 'bg-orange-500' : 'bg-gray-300'}`}
            >
              <div className={`bg-white w-5 h-5 rounded-full shadow-md transform transition-transform duration-300 ease-in-out ${darkMode ? 'translate-x-7' : ''}`}></div>
            </button>
          </div>
        </div>
        ...
        <div className="bg-white p-8 rounded-3xl border border-red-100 shadow-lg shadow-red-900/5 relative overflow-hidden mt-8">
          ...
          <p className="text-sm text-gray-500 mb-6">Hesabınızı silmek geri alınamaz bir işlemdir. Tüm biletleriniz kaybolur.</p>
          <button className="bg-red-50 text-red-600 font-bold px-6 py-3 rounded-xl border border-red-200 hover:bg-red-600 hover:text-white transition-all">
            Hesabımı Sil
          </button>
        </div>
