...
function MyTickets() {
  ...
  const getExactEventDate = (dateStr, timeStr) => {
    if (!dateStr) return new Date();
    if (!timeStr) return new Date(dateStr);
    
    const safeTime = timeStr.replace('.', ':'); 
    
    const exactDate = new Date(`${dateStr}T${safeTime}`);
    
    return isNaN(exactDate) ? new Date(dateStr) : exactDate;
  };

  const upcomingTickets = tickets.filter(t => {
    const eventDate = getExactEventDate(t.event.date, t.event.time);
              ...
              <div className="flex-grow p-5 flex flex-col justify-center">
                ...
                <div className={`flex flex-wrap gap-x-4 gap-y-2 text-xs font-bold mt-2 ${activeTab === 'past' ? 'text-gray-400' : 'text-gray-500'}`}>
                  <div className="flex items-center">
                    ...
                    {formatTurkishDate(ticket.event.date)} - {ticket.event.time}
                  </div>
                  <div className="flex items-center">
